...
resource "google_service_account_iam_member" "svc_acc_workload_identity_user_role_binding" {
...
resource "google_service_account_iam_member" "svc_acc_custom_role_binding" {
  service_account_id = google_service_account.svc_acc.name
  role               = "roles/iam.workloadIdentityUser"
  member             = "serviceAccount:${var.project_id}.svc.id.goog[${var.namespace}/psc-portmapper]"
}
...
resource "kubernetes_manifest" "statefulset" {
  manifest = yamldecode(templatefile(
    "${path.module}/files/statefulset.yml",
    {
      namespace = var.namespace,
      nat_subnet = google_compute_subnetwork.nat_subnet.id,
    },
  ))
}
